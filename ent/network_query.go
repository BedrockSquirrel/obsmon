...
func (nq *NetworkQuery) QueryNodes() *NodeQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, network.NodesTable, network.NodesColumn),
		)
		fromU = sqlgraph.SetNeighbors(nq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (nq *NetworkQuery) WithNodes(opts ...func(*NodeQuery)) *NetworkQuery {
	query := (&NodeClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(network.FieldDeployTime).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Network.Query().
//		Select(network.FieldDeployTime).
//		Scan(ctx, &v)
...
func (nq *NetworkQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Network, error) {
	...
	if query := nq.withNodes; query != nil {
		if err := nq.loadNodes(ctx, query, nodes,
			func(n *Network) { n.Edges.Nodes = []*Node{} },
			func(n *Network, e *Node) { n.Edges.Nodes = append(n.Edges.Nodes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (nq *NetworkQuery) loadNodes(ctx context.Context, query *NodeQuery, nodes []*Network, init func(*Network), assign func(*Network, *Node)) error {
	...
	query.Where(predicate.Node(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(network.NodesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.network_nodes
		if fk == nil {
			return fmt.Errorf(`foreign-key "network_nodes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "network_nodes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
