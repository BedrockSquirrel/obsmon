...
import (
	...
	"github.com/bedrocksquirrel/obsmon/ent/migrate"
...
// QueryNodes queries the nodes edge of a Network.
func (c *NetworkClient) QueryNodes(n *Network) *NodeQuery {
...
// QueryNetwork queries the network edge of a Node.
func (c *NodeClient) QueryNetwork(n *Node) *NetworkQuery {
	query := (&NetworkClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := n.ID
		step := sqlgraph.NewStep(
...
type (
	hooks struct {
		Environment, Network, Node []ent.Hook
	}
	inters struct {
		Environment, Network, Node []ent.Interceptor
	}
)
