...
func (nq *NodeQuery) QueryNetwork() *NetworkQuery {
	query := (&NetworkClient{config: nq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, node.NetworkTable, node.NetworkColumn),
		)
...
func (nq *NodeQuery) Clone() *NodeQuery {
	...
	return &NodeQuery{
		...
		withNetwork: nq.withNetwork.Clone(),
...
func (nq *NodeQuery) WithNetwork(opts ...func(*NetworkQuery)) *NodeQuery {
	query := (&NetworkClient{config: nq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	nq.withNetwork = query
	return nq
}
...
func (nq *NodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Node, error) {
	...
	if query := nq.withNetwork; query != nil {
		if err := nq.loadNetwork(ctx, query, nodes, nil,
			func(n *Node, e *Network) { n.Edges.Network = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (nq *NodeQuery) loadNetwork(ctx context.Context, query *NetworkQuery, nodes []*Node, init func(*Node), assign func(*Node, *Network)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Node)
	for i := range nodes {
		if nodes[i].network_nodes == nil {
			continue
		}
		fk := *nodes[i].network_nodes
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "network_nodes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
